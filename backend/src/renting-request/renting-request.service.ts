...
export class RentingRequestService {
  ...
  async providergetrequest(
    ...
    for (let i = 0; i < rentingRequests.length; i++) {
      const providerrequest = new OutputProviderPageDto();
      ...
      providerrequest.registrationId =
        rentingRequests[i].vehicle.registrationId;
      ...
      providerrequest.maximumCapacity =
        rentingRequests[i].vehicle.maximumCapacity;
  ...
  async rentergetrequest(renter_id: number): Promise<OutputRenterPageDto[]> {
    ...
    renterrequests = [];
    for (let i = 0; i < rentingRequests.length; i++) {
      const renterrequest = new OutputRenterPageDto();
      ...
      renterrequest.provider_firstname =
        rentingRequests[i].vehicle.user.first_name;
      renterrequest.provider_lastname =
        rentingRequests[i].vehicle.user.last_name;
      renterrequest.registrationId = rentingRequests[i].vehicle.registrationId;
      renterrequest.maximumCapacity =
        rentingRequests[i].vehicle.maximumCapacity;
